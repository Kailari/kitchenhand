...
import { PageWithHeadingAndBreadcrumb } from '../PageBase';

const DiscoverPage = ({ breadcrumbs }) => (
  <PageWithHeadingAndBreadcrumb title='Discover new recipes' breadcrumbs={breadcrumbs}>
    ...
    <Segment color='olive' stacked style={{ paddingBottom: 0 }}>
      <RecipesQuery query={NEW_RECIPES} render={result => result.loading
    ...
    <Segment color='olive' stacked>
      <RecipesQuery query={TRENDING_RECIPES} render={result => result.loading
        ? <Loader active inline='centered'>Loading...</Loader>
        : <Carousel
